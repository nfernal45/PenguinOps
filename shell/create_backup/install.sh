...
if [ $# -lt 1 ]
then
  echo "Введите 1 или несколько полных или относительных путей до директорий. Например \"./install /nfs /home /opt\"" 
  exit 1
fi
...
sed -i "s|ExecStart=.*|ExecStart=${NEW_EXEC_START_PATH} ${SOURCE_DIRS}|" "$(pwd)/systemctl/${SERVICE_NAME}.service"
